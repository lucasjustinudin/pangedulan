...
def save_user_data(chat_id: int):
    ...
    save_local_user_data(chat_id, data_to_save)

def load_user_data(chat_id: int) -> dict | None:
    loaded_data = load_local_user_data(chat_id)
    if loaded_data:
        logger.info(f"Data pengguna {chat_id} berhasil dimuat dari file lokal.")
        return loaded_data
    else:
        logger.info(f"Tidak ada data tersimpan di file lokal untuk pengguna {chat_id}.")
        return None
...
async def handle_message(update: Update,
    ...
    if chat_id not in user_data:
        ...
        if loaded_data:
            ...
            logger.info(f"Memuat kembali data untuk {chat_id} dari file lokal.")
        else:
    ...
    try:
        if data['session'] is None:
            ...
            initial_history = [
                ...
                {'role': 'model', 'parts': ["Wih, anjay, si bos sumping! Sini, uy, geser ka dieu. Geus mesen ngopi can? Aing ti tadi gabut pisan. Enaknya kumaha yeuh urang?"]},
            ]
